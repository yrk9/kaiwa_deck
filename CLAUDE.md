...
- **コミットは人間が手動で行う。** Claudeはコミットしない。変更を終えたら内容を説明し、人間の判断に委ねる
- **コードは1コミット分の粒度ずつ出力する。** 依存追加・機能実装・ドキュメント更新のように複数の関心事を一度に進めず、区切りごとに人間が確認・コミットしてから次に進む
- **git追跡外のファイル・ディレクトリを削除・追加・編集する場合は、必ず事前にユーザーに確認する。**

### コード

- **Pythonは PEP 8 に則る。** 出力前に準拠を確認する(命名、1行の長さ、import順など)
- **テストコードの冒頭に、何を確認しているか(確認していないことも)を書く。**

### 文章(ドキュメント・コメント)

- **網羅的に書きすぎない。** 読み手は大学生〜ジュニアエンジニアを想定し、要点を絞って平易・簡潔に書く。全パターンの列挙や過剰な詳細は避ける。コードのコメントも同様
